 ...
 * @modified.v=1:
 *----------------------------------------------------------------------
 * L.Draw.Marker#removeHooks() で _map のイベント off が
 * 行われていない。
 *
 * - L.Draw.Marker#removeHooks()
 *     this._marker は作成されていない場合があるため、
 *     this._map.off() は this._marker の有無に関係無く行う。
 *
 * @modified.v=2:
 *----------------------------------------------------------------------
 * ポリゴン作図中に 2 点しかプロットしていないのに completeShape() 呼び出しで
 * 作図が確定してしまう。
 *
 * - L.Draw.Polyline#completeShape()
 *     点のプロット数を満たしているかの判断を this._markers.length ではなく
 *     _shapeIsValid() で行う
 * - L.Draw.Polyline#_shapeIsValid()
 *     常に true を返すのをやめ、2 点以上の場合に true を返す。
 *
 * @modified.v=3:
 *----------------------------------------------------------------------
 ...
 * @modified.v=6:
 *----------------------------------------------------------------------
 * NOTE: touch: L.Edit.PolyVerticesEdit で touchend での _fireEdit が行われていないようなので、
 *              _onTouchMove 時にも _fireEdit を呼び出す。
 */
