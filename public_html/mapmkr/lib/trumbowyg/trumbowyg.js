 ...
 * @modified.v=1:
 *----------------------------------------------------------------------
 * # getTagsRecursive で例外が起きるのを抑止した。
 * # html() 呼び出し時に内容が反映されていない場合があるので、syncTextarea() を呼び出すようにした。
 * # semantic オプションを false にした際に、デフォルトのボタンが
 *   違うボタングループと置き換えられていたのを修正。
 *   t.o.btns[4] = 'btnGrp-design'; => t.o.btns[3] = 'btnGrp-design';
 * # オーバーレイの高さを表示時にも設定するようにコードを追加。
 * # 画像の挿入で独自のダイアログ画面を表示するように変更した。
 * # リンクの作成で独自のダイアログ画面を表示するように変更した。
 * # テンプレートの挿入機能を追加した。
 * # viewHTML 時のコールバックを追加した。
 * # html ソースのスクリプト除外処理を追加。ただし、本格的にスクリプトを除外するためではなく、
 *   編集中に javascript が実行されるのを(できるだけ)抑制するため。
 */
        ...
        if (this.length === 1) {
            try {
                ...
                switch (options) {
                    ...
                    // テンプレートの挿入
                    case 'insertTemplate':
                        return t.insertTemplate(params);
                }
            } catch (c) {
            }
        }
    ...
    var Trumbowyg = function (editorElem, options) {
        ...
        t.o = $.extend(true, {}, {
            ...
            imgDblClickHandler: function () {
                ...
                var insertImageDialog = new MKR.Trumbowyg.Dialog.ModalInsertImage(t.o.dialogManager, $img.attr('src'), $img.attr('alt'), $img.css('width'));
                insertImageDialog.on("mkrDialog:ok", function (event) {
                    ...
                    $img.css({'width': event.width});
                });
                insertImageDialog.on("mkrDialog:cancel", function (event) {
                    t.endModal();
                });
            ...
            viewHTMLCallback: null  // function (htmlVisiblity) {}: htmlVisiblity: ソース表示時に true
        }, options);
    ...
    Trumbowyg.prototype = {
        ...
        toggle: function () {
            ...
            setTimeout(function () {
                ...
                if (t.$box.hasClass(prefix + 'editor-visible')) {
                    t.o.viewHTMLCallback(false);
                    t.$ta.attr('tabindex', -1);
                } else {
                    t.o.viewHTMLCallback(true);
        ...
        syncCode: function (force) {
            ...
            if (!force && t.$ed.is(':visible')) {
                ...
                t.$ed.html(t.htmlSanitize(t.$ta.val()));
            }
        ...
        htmlSanitize: function (html) {
            var domArr = $.parseHTML(html, document, false);
            var htmlNew = $("<div></div>").append(domArr).html();
            return htmlNew;
        },
        ...
        createLink: function () {
            ...
            // NOTE: 引数 text に getRangeText() の文字列を渡しているのは、選択されている文字列がある場合は、それをテキスト文字列として表示するため。
            var createLinkDialog = new MKR.Trumbowyg.Dialog.ModalCreateLink(t.o.dialogManager, url, t.getRangeText(), title, target);
            createLinkDialog.on("mkrDialog:ok", function (event) {
                t.endModal();
                ...
                link.attr({
                    'href': event.url,
                    'title': event.title,
                    'target': event.aTarget
                });

                t.range.deleteContents();
                t.range.insertNode(link[0]);
            });
            createLinkDialog.on("mkrDialog:cancel", function (event) {
                t.endModal();
            });
        ...
        insertTemplate: function (params) {
            ...
            t.range.insertNode(endBr[0]);
            t.range.insertNode(endHr[0]);
            t.range.insertNode(node);
            t.range.insertNode(beginHr[0]);
            t.range.insertNode(beginBr[0]);
        },
        insertImage: function () {
            ...
            // NOTE: 引数 alt に getRangeText() の文字列を渡しているのは、選択されている文字列がある場合は、それを alt 文字列として表示するため。
            var insertImageDialog = new MKR.Trumbowyg.Dialog.ModalInsertImage(t.o.dialogManager, "", t.getRangeText(), "");
            insertImageDialog.on("mkrDialog:ok", function (event) {

                t.endModal();

                t.restoreRange();
                t.nativeExecCmd('insertImage', event.url);

                // NOTE: 同じ src を持ち alt がない img も書き換えてしまう
                var $img = $('img[src="' + event.url + '"]:not([alt])', t.$box);
                $img.attr('alt', event.alt);
                $img.css({'width': event.width});
            });
            insertImageDialog.on("mkrDialog:cancel", function (event) {
                t.endModal();
        ...
        /*
         * call native execCommand
         */
        nativeExecCmd: function (cmd, param) {
            var t = this;
